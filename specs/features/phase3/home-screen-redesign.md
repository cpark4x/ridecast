...
> **Implementation plan:** `docs/plans/2026-03-10-home-screen-implementation-plan.md`

# Home Screen Redesign — Dev-Machine Spec

## Motivation

The current Home tab is a generic queue list with no visual distinction between episodes, no indication of what has already been listened to, and navigation real estate wasted on Upload and Settings tabs that most users rarely touch. The Library tab already covers the full content list, making the Home tab's "Your Queue" view redundant and adding friction rather than delighting the user. The redesign turns Home into a genuinely useful daily-drive screen — time-based greeting, one-tap "Play All" with auto-advance, progress indicators on partially-listened episodes, and a compact 2-tab nav with a FAB for upload — removing clutter and making the app feel intentional.
...
|---|---|---|
| API: PlaybackState join | `src/app/api/library/route.ts` | Return listened/position data per episode |
| PlayerContext: userId fix | `src/components/PlayerContext.tsx` | Fix hardcoded "default-user" to use Clerk |
| PlayerContext: queue | `src/components/PlayerContext.tsx` | Add playQueue(), auto-advance, skipToNext/Previous |
| Upload: shorter presets | `src/components/UploadScreen.tsx` | Add 2min/3min presets, lower slider min |
| Nav restructure | `src/components/BottomNav.tsx`, `src/components/AppShell.tsx` | 2 tabs + FAB + gear icon |
| Shared utils | `src/lib/ui/content-display.ts` (create) | Extract gradients, timeAgo, source icons, title fallback |
| HomeScreen rewrite | `src/components/HomeScreen.tsx` | Full rewrite per Daily Drive design |
| Title fallback | `src/lib/ui/content-display.ts` | Derive title from URL domain or sourceType when blank |
...
| File | Purpose |
|---|---|
| `src/lib/ui/content-display.ts` | Shared UI utilities |
| `src/lib/ui/__tests__/content-display.test.ts` | Tests for shared utilities |

**Modify:**

| File | Purpose |
|---|---|
| `src/app/api/library/route.ts` | Join PlaybackState, return author |
| `src/components/PlayerContext.tsx` | Fix userId, add queue support |
| `src/components/UploadScreen.tsx` | Shorter duration presets |
| `src/components/BottomNav.tsx` | 2 tabs + FAB |
| `src/components/AppShell.tsx` | Modal overlays for Upload/Settings |
...
- Modify: `src/components/AppShell.test.tsx` — existing tests **will break**; replace entirely

### Step 1: Update AppShell.test.tsx with explicit assertions for the new structure

The existing `src/components/AppShell.test.tsx` (read before implementing) has 3 tests that will all break after this task:

1. **"renders bottom nav with four tabs (Home, Upload, Library, Player)"** — breaks: tab count changes to 2
2. **"shows upload screen by default"** — breaks: default tab changes to `"home"`, Upload becomes a modal
3. **"switches to library screen when Library tab is clicked"** — survives unchanged

Replace the **entire** contents of `src/components/AppShell.test.tsx` with:
...
describe("AppShell", () => {
  ...
  it("shows home screen by default (not upload)", async () => {
    renderWithPlayer();
    // Default tab is "home" — UploadScreen "Drop files here" should NOT be the default view
    await waitFor(() => {
  ...
  it("clicking gear icon opens Settings overlay showing 'Settings' heading", async () => {
    ...
    await waitFor(() => {
      // SettingsScreen renders <span className="text-sm font-semibold">Settings</span>
...
**Expected:** All new AppShell tests pass. All UploadScreen tests pass.
...
```bash
git add src/components/BottomNav.tsx src/components/AppShell.tsx src/components/AppShell.test.tsx
...
> **Path note:** The canonical path for the shared module is `src/lib/ui/content-display.ts` and `src/lib/ui/__tests__/content-display.test.ts` (as specified in the Files Summary). All imports in HomeScreen, LibraryScreen, and test files should use `@/lib/ui/content-display`.

**Files:**
- Create: `src/lib/ui/content-display.ts`
- Create: `src/lib/ui/__tests__/content-display.test.ts`
...
```bash
npx vitest run src/lib/ui/__tests__/content-display.test.ts
```
...
```bash
git add src/lib/ui/content-display.ts src/lib/ui/__tests__/content-display.test.ts src/components/LibraryScreen.tsx
...
**NOTE:** This was already implemented in Task 6 as `getTitleFallback()` in `src/lib/ui/content-display.ts`, and it's already used in the HomeScreen from Task 7. This task verifies the integration works end-to-end and applies the same fallback to LibraryScreen.
...
```typescript
import { gradients, sourceIcons, timeAgo, getTitleFallback } from "@/lib/ui/content-display";
```
...
- [ ] `GET /api/library` returns `completed` and `position` for each version
- [ ] `GET /api/library` returns `author` for each content item
- [ ] PlayerContext uses Clerk userId, not "default-user"
- [ ] `playQueue([items])` plays first item and auto-advances on ended
- [ ] `skipToNext()` and `skipToPrevious()` work correctly
- [ ] Upload presets include 2min and 3min options, slider min is 2
- [ ] BottomNav shows exactly 2 tabs: Home and Library
- [ ] FAB "+" button opens Upload as modal overlay
- [ ] Gear icon opens Settings as modal overlay
- [ ] HomeScreen shows time-based greeting
- [ ] HomeScreen shows "Play All" button that calls playQueue with fitting episodes
- [ ] Currently playing card appears when audio is active with progress bar
- [ ] Up Next shows only unlistened episodes
- [ ] Episode cards show color-coded gradients, source-type icons, sourceType + timeAgo subtitle
- [ ] No "narrator" label or "X min target" text anywhere on HomeScreen
...
| What | Why |
|---|---|
| Content discovery pipeline ("For You" section backend) | No content sources exist yet; UI placeholder designed but backend deferred |
| Newsletter/RSS subscriptions | Separate Phase 3 feature |
| Native mobile app | Separate Phase 2 feature |
| Browser extension | Separate Phase 2 feature |
| Sort controls | Single sort order (newest first) is sufficient for now |
