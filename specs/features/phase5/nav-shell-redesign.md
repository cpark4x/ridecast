...
//   Container borderRadius:    16  → 14  (borderRadius.miniPlayer)
//   Container backgroundColor: '#1c1c1e' → colors.surfaceElevated ('#242438')
//   Container shadow props:    REMOVE (elevation model uses color, not shadow)
//   Thumbnail size prop:       36  → 40
//   Title fontSize:            13  → 14  (typography.sizes.body - 1)
//   Title fontWeight:          '600' → '500' (typography.weights.medium)
//   Title color:               '#fff' → '#F5F5F5'
//   Caption fontSize:          11  → 12  (typography.sizes.caption)
//   Caption color:             'rgba(255,255,255,0.5)' → colors.textSecondary ('#9CA3AF')
//   Progress bar fill color:   '#EA580C' → colors.accentPrimary ('#FF6B35')
//   Play/pause icon size:      22  → 24  (sizes.iconNav)
//   Skip controls (rewind-15, skip-30): REMOVE — mini player spec shows play/pause only

// Style values that are unchanged:
//   marginHorizontal: 8        (8px side margins as per nav-shell spec)
...
// Exempt route segments (PlayerBar hidden):
const EXEMPT_SEGMENTS = ['sign-in', 'processing', 'settings'] as const;
...
- Container: `#242438` background, `14px` corner radius, `8px` horizontal margin from screen edges. No shadow or elevation props of any kind (`shadowColor`, `shadowOffset`, `shadowOpacity`, `shadowRadius`, `elevation` are all removed).
- Thumbnail: `40px` square rendered via `<SourceThumbnail size={40} ... />`.
...
| AC-12 | `PlayerBar` has no shadow props (`shadowColor`, `elevation`, etc.) | `native/__tests__/PlayerBar.test.tsx` — assert container View style does NOT include any shadow property |
| AC-13 | `PlayerBar` thumbnail is rendered at `size={40}` | `native/__tests__/PlayerBar.test.tsx` — inspect `SourceThumbnail` prop `size` |
...
| `native/components/PlayerBar.tsx` | **Modify** (visual refresh) | Apply new style values per §2 Interfaces table. Remove `shadowColor`, `shadowOffset`, `shadowOpacity`, `shadowRadius`, `elevation`. Remove rewind-15 and skip-30 `TouchableOpacity` blocks. Change thumbnail `size` to 40. Update text and color values. ~30 LOC net change. |
| `native/app/_layout.tsx` | **Modify** | Add `useSegments()` call inside `AppShell`. Add `EXEMPT_SEGMENTS` constant. Wrap `<PlayerBar />` with `{!isExemptScreen && <PlayerBar />}`. Import `useSegments` (already imported in file via `expo-router`). ~8 LOC net change. |
...
| `native/__tests__/PlayerBar.test.tsx` | **Create** | Tests for AC-10 through AC-21. Mock `usePlayer` with a `currentItem`, mock `useSegments`. Assert style props and absence of skip controls. ~80 LOC. |
...
- **No shadows anywhere.** The aesthetic brief explicitly forbids shadow elevation. `PlayerBar` currently has `shadowColor`, `shadowOffset`, `shadowOpacity`, `shadowRadius`, and `elevation` — all must be removed. Depth is communicated by `#242438` (surfaceElevated) floating above `#0F0F1A` (backgroundScreen). See `aesthetic-brief.md` → Anti-Slop Notes.
...
- **Nav-shell.md lists 2 tabs; aesthetic-brief.md lists 3.** The aesthetic brief is the authoritative source. The nav-shell storyboard was written before Discover was finalized. The 3-tab layout (Home / Discover / Library) is correct per `aesthetic-brief.md` line 95.
- **`nav-shell.md` includes a shadow spec for the mini player** (`0 4px 16px rgba(0,0,0,0.3)`). This contradicts the aesthetic brief Anti-Slop rule "No shadow elevation." The aesthetic brief takes precedence. Shadow is not implemented.
...
- **`AppShell` uses `useSegments` independently from `AuthGate`.** Both components call `useSegments()` in the same render tree — this is safe. `useSegments` is a hook that reads from the Router context.
...
- **Deferred:** per-tab label `fontFamily` (Geist) is not set here. Font loading is a separate concern. The tab labels will use the system font until a dedicated font-loading spec is implemented.
...
| Remove shadow props from PlayerBar container | `native/components/PlayerBar.tsx` — outer `<View style={{...}}>` | Inline style — remove keys: `shadowColor`, `shadowOffset`, `shadowOpacity`, `shadowRadius`, `elevation` | Grep current file to confirm exact keys before removing |
...
| Update PlayerBar thumbnail size to 40 | `native/components/PlayerBar.tsx` — `<SourceThumbnail size={...}>` | `SourceThumbnail` `size` prop — currently `36`, change to `40` | Confirm `SourceThumbnail` accepts `size` as a number prop (it does per double-logo spec) |
...
| Add `useSegments` to `AppShell` | `native/app/_layout.tsx` — `AppShell` component body | `useSegments` from `expo-router` — already imported at file top; no new import needed | Verify `useSegments` is in the existing import statement on line 5 |
| Wrap PlayerBar with exempt-screen guard | `native/app/_layout.tsx` — `AppShell` JSX, `<PlayerBar />` line (~line 90) | `segments[0]` string comparison against `EXEMPT_SEGMENTS` array | Confirm `segments` type is `string[]` — `useSegments()` return type |
